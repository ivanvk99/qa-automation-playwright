...
test.describe('Login page UI', () => {
  ...
  test('submitting an empty form shows a validation message', async ({ page }) => {
    ...
    const errorMsg = page.locator('[class*="error"], [class*="alert"], [class*="invalid"], .help-block, .text-danger').first();
  ...
  test('login page has a link to registration or password reset', async ({ page }) => {
    await gotoApp(page, '/#/login');
    const extraLink = page.locator('a').filter({ hasText: /register|sign up|forgot|reset/i }).first();
